<?php include($_SERVER['DOCUMENT_ROOT']."/includes/head.inc.php"); ?>

<?php include($_SERVER['DOCUMENT_ROOT']."/includes/nav.inc.php"); ?>

<link href="css/bootstrap.min.css" rel="stylesheet">
    <!-- Custom CSS-->
    <link href="css/main.css" rel="stylesheet">

    <header class="intro introhalf" data-background="/assets/img/r6_team.jpg">
      <div class="intro-body">
      </div>
	</header>  

	<section class="bg-gray text-center">
    <div class="container wow fadeIn">
      <div class="row justify-content-center">
      <div class="col-md-2 col-sm-6">
            <p><img class="img-responsive center-block" src="/assets/img/default.png" alt=""></p>
            <h2 class="classic">NMS Zoltan</h2>
            <ul class="list-inline">
              <li><a href="https://twitter.com/Zoltan_R6"><i class="fab fa-twitter fa-lg"></i></a></li>
            </ul>
            <h6>Captain</h6>
          </div>
          <div class="col-md-2 col-sm-6">
            <p><img class="img-responsive center-block" src="/assets/img/default.png" alt=""></p>
            <h2 class="classic">NMS F1re</h2>
            <ul class="list-inline">
              <li><a href="https://twitter.com/F1RE_R6"><i class="fab fa-twitter fa-lg"></i></a></li>
            </ul>
            <h6>Player</h6>
          </div>
          <div class="col-md-2 col-sm-6">
            <p><img class="img-responsive center-block" src="/assets/img/default.png" alt=""></p>
            <h2 class="classic">NMS Yung</h2>
            <ul class="list-inline">
              <li><a href="https://twitter.com/yungisBAD"><i class="fab fa-twitter fa-lg"></i></a></li>
            </ul>
            <h6>Player</h6>
          </div>
          <div class="col-md-2 col-sm-6">
            <p><img class="img-responsive center-block" src="/assets/img/default.png" alt=""></p>
            <h2 class="classic">NMS Stisko</h2>
            <ul class="list-inline">
              <li><a href="https://twitter.com/Michalastn3"><i class="fab fa-twitter fa-lg"></i></a></li>
            </ul>
            <h6>Player</h6>
          </div>
          <div class="col-md-2 col-sm-6">
            <p><img class="img-responsive center-block" src="/assets/img/default.png" alt=""></p>
            <h2 class="classic">NMS VeterAn</h2>
            <ul class="list-inline">
              <li><a href="https://twitter.com/R6VeterAn"><i class="fab fa-twitter fa-lg"></i></a></li>
            </ul>
            <h6>Player</h6>
          </div>
          <div class="col-md-2 col-sm-6">
            <p><img class="img-responsive center-block" src="/assets/img/default.png" alt=""></p>
            <h2 class="classic">NMS Voya</h2>
            <ul class="list-inline">
              <li><a href="https://twitter.com/the_voya"><i class="fab fa-twitter fa-lg"></i></a></li>
            </ul>
            <h6>Sub</h6>
          </div>
        </div>
      </div>
    </section>

<?php include($_SERVER['DOCUMENT_ROOT']."/includes/foot.inc.php"); ?>