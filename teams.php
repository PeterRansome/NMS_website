<?php include($_SERVER['DOCUMENT_ROOT']."/includes/head.inc.php"); ?>

<?php include($_SERVER['DOCUMENT_ROOT']."/includes/nav.inc.php"); ?>
<link href="css/bootstrap.min.css" rel="stylesheet">
    <!-- Custom CSS-->
    <link href="css/main.css" rel="stylesheet">


<header class="intro introhalf" data-background="/assets/img/bg-img1.png">
      <div class="intro-body">
        <h2>Our Teams</h2>
      </div>
</header>

 <section class="section-small portfolio-wide" id="portfolio">
        <div class="container">
        <div class="row portfolio-items">
            <div class="col-sm-6">
              <div class="portfolio-item"><a href="/csgo"><img src="/assets/img/csgo_team.jpg" alt="">
              </a>
            </div>
            </div>
            <div class="col-sm-6">
              <div class="portfolio-item"><a href="/valorant"><img src="/assets/img/valorant_team.jpg" alt="">
              </a>
              </div>
            </div>

          <div class="col-sm-6">
                <div class="portfolio-item"><a href="/fortnite"><img src="/assets/img/fortnite.jpg" alt="">
                </a>
                </div>
          </div>
          <div class="col-sm-6">
                <div class="portfolio-item"><a href="/rocket-league.php"><img src="/assets/img/r6_team.jpg" alt="">
                </a>
                </div>
          </div>
          <div class="col-sm-6">
                <div class="portfolio-item"><a href="/fifa"><img src="/assets/img/fifa_team.jpg" alt="">
                </a>
                </div>
          </div>
          <div class="col-sm-6">
                <div class="portfolio-item"><a href="/content-creators"><img src="/assets/img/content_team.jpg" alt="">
                </a>
                </div>
          </div>
          <div class="col-sm-6">
                <div class="portfolio-item"><a href="/management"><img src="/assets/img/management_team.jpg" alt="">
                </a>
                </div>           
          </div>
         </div>
        </div>
      </div>
    </section>

<?php include($_SERVER['DOCUMENT_ROOT']."/includes/foot.inc.php"); ?>